...
public class Trol_Pool_Manager : MonoBehaviour {
	...
	public void LaunchEnemy(Vector3 Caller,bool beast)
	{
		if(claw_enemy_list != null)
		{
			for(int i = 0;i < claw_enemy_list.Count;i++)
			{
				...
				if(!claw_enemy_list[i].activeInHierarchy)
				{
					...
					claw_enemy_list[i].GetComponent<Trol>().Become_Normal();
					if(beast)
						claw_enemy_list[i].transform.SetParent(null);
